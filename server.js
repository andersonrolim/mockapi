...
const AUTH_ENABLED         = !!(GITHUB_CLIENT_ID || GOOGLE_CLIENT_ID);
...
<div class="container">
  ...
  if (method === 'GET' && pathname === '/auth/google/callback') {
    const code = parsed.query.code;
    if (!code) { res.writeHead(302, { Location: '/login?error=google_nocode' }); res.end(); return; }
    try {
      const base     = getBaseUrl(req);
      ...
      const tokenData = await tokenRes.json();
      if (!tokenData.access_token) throw new Error('No Google access token: ' + JSON.stringify(tokenData));
      ...
      if (!gUser.id) throw new Error('No Google user id');

      // Build stable userId and login from Google profile
      const userId  = 'G' + String(gUser.id);
      const login   = (gUser.email || gUser.id).split('@')[0].replace(/[^a-zA-Z0-9_-]/g, '-');
      const isAdmin = db.countUsers() === 0;

      const user = db.upsertGoogleUser({
        id: userId, googleId: String(gUser.id), login,
        name: gUser.name || login, email: gUser.email || null,
        avatar: gUser.picture || null, plan: 'free', isAdmin,
      });
      ...
      // Auto-accept pending workspace invites
      const pendingInvites = db.getPendingInvitesByLogin(login);
      for (const invite of pendingInvites) {
        db.addWorkspaceMember(invite.workspace_id, user.id, 'editor', invite.invited_by);
        db.deleteInvite(invite.id);
      }
      ...
      console.error('[auth] Google callback error:', e.message, e.stack);
      if (!res.headersSent) { res.writeHead(302, { Location: '/login?error=google_fail' }); res.end(); }
    }
    return;
  }
